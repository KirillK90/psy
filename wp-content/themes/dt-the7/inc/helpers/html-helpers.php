...
if ( ! function_exists( 'presscore_get_share_buttons_list' ) ) :
	...
	function presscore_get_share_buttons_list( $place, $post_id = null ) {
		...
		foreach ( $buttons as $button ) {
			...
			$share_button = '<a class="' . $icon_class . '" href="' . esc_url( $url ) . '" title="' . esc_attr( $desc ) . '" target="_blank"' . $custom . '><span class="screen-reader-text">' . sprintf( __( 'Share with %s', 'the7mk2' ), $desc ) . '</span></a>';
